...
import { Gift, Medal, Ruler, ScrollText, Shield, Sticker } from "lucide-react";
...
const MORE_LINKS = [
  ...
  { href: "/canhoes/amigo-secreto", icon: <Gift className="h-4 w-4" />, label: "Amigo Secreto" },
  { href: "/canhoes/gala", icon: <Medal className="h-4 w-4" />, label: "Gala" },
  { href: "/canhoes/medidas", icon: <Ruler className="h-4 w-4" />, label: "Medidas" },
  { href: "/canhoes/nomeacoes", icon: <ScrollText className="h-4 w-4" />, label: "Nomeações" },
] as const;
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="pb-safe">
        ...
        <div className="grid gap-3 px-4 pb-4 sm:grid-cols-2">
          {MORE_LINKS.map((link) => (
            <button
              ...
              className="canhoes-tap flex min-h-12 items-center gap-3 rounded-2xl border border-[var(--color-beige-dark)]/25 bg-[var(--color-bg-surface-alt)] px-4 py-3 text-left text-[var(--color-text-dark)]"
            >
        ...
        <div className="px-4 pb-4 pt-4">
          <Button variant="secondary" className="w-full justify-center" onClick={() => onNavigate("/canhoes/admin")}>
